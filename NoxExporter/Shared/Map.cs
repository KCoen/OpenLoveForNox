...
namespace NoxShared
{
	public class Map
	{
		...
		public static ThingDb thingdb;

		// ----CONSTRUCTORS----
		public Map() { }

		public Map(string filename, ThingDb thingdb) : this()
		{
			Map.thingdb = thingdb;
			Load(filename);
		}
		...
		public class WallMap : SortedDictionarySection<Point, Wall>
		{
			...
			protected override void ReadContents(Stream stream)
			{
				...
				if (finish != rdr.BaseStream.Position)
				{
					...
					Debug.WriteLine("Detected Fake-Terminator bug, forcing reading..");
					goto repeat;
				}
			}
		...
		public class MapIntro : Section
		{
			...
			protected override void ReadContents(Stream stream)
			{
                ...
                if (Text.Length > 0)
                	Debug.WriteLine(string.Format("MapIntro section text: {0}", Text));
            }
		...
		public class Object : IComparable, ICloneable
		{
			...
			public ArrayList InventoryList = new ArrayList(); //Objects in its inventory
			public string pickup_func = ""; //Function to execute when picked-up
			public List<UInt32> SlaveGIDList = new List<uint>();//FF term. unknown (Used by SaveGameLocation) 
			...
			public object ObjXfer;
			public ObjXfer.MonsterGeneratorXfer MonsterGenData = null;
			...
			internal Object(Stream stream, IDictionary toc)//read an object from the stream, using the provided toc to identify the object
			{
				Extent = -1;
				Read(stream, toc);
			}
			...
			internal void Read(Stream stream, IDictionary toc)
			{
				...
				{
					Debug.WriteLine(string.Format("({0}: RR 0x{1:X}) Unsupported entry structure.\n Ignoring this object...", Name, ReadRule2));
				...
				if (rdr.BaseStream.Position <= endOfData)
				{
					...
					if(thingdb.Things[Name].Xfer != null)
						ObjXfer = XfersHelper.GetXfer(thingdb.Things[Name].Xfer, this);

					/*if (thingdb.Things[Name].Xfer == "MonsterGeneratorXfer")
					{
						// Monster generator data is offset dependent
						long attemptPos = rdr.BaseStream.Position;
						Debug.WriteLine(String.Format("Parsing MonsterGeneratorXfer for object: {0}", this));
						try
						{
							MonsterGenData = ObjXfer.MonsterGeneratorXfer.FromStream(rdr.BaseStream, ReadRule1);
							read = true;
						}
						catch (Exception)
						{
							Debug.WriteLine("Failed to parse MonsterGeneratorXfer!");
							rdr.BaseStream.Seek(attemptPos, SeekOrigin.Begin);
						}
					}*/
				}
				else
				...
				if (inven > 0)
				{
					InventoryList = new ArrayList(inven);
			...
			internal void Write(Stream stream, IDictionary toc)
			{
				...
				if (Terminator == 0xFF)
				{
					...
					if (ReadRule2 >= 0x3F)
					{
						...
						if (ReadRule2 >= 0x40)
						{
							wtr.Write(DestroyFrame);
						}
					}
                }

				if (thingdb.Things[Name].Xfer == "MonsterGeneratorXfer" && MonsterGenData != null)
				{
					Debug.WriteLine(String.Format("Writing MonsterGeneratorXfer for object: {0}", this));
					MonsterGenData.WriteToStream(wtr.BaseStream, ReadRule1);
				}
				else if (modbuf != null)
					wtr.Write(modbuf);
				...
				foreach (Object o in InventoryList)
					o.Write(stream, toc);
			}
			...
			public object Clone()
			{
				...
				copy.CreateFlags = CreateFlags;
				copy.InventoryList = new ArrayList(InventoryList.Count);
				foreach (Object o in InventoryList)
					copy.InventoryList.Add(o.Clone());
				copy.Extent = Extent;
				...
				copy.ReadRule1 = ReadRule1;
				copy.ReadRule2 = ReadRule2;
				copy.Scr_Name = Scr_Name;
				copy.Team = Team;
				copy.AnimFlags = AnimFlags;
				copy.DestroyFrame = DestroyFrame;
				copy.Terminator = Terminator;
				copy.IngameID = IngameID;
				return copy;
			}
		}
		...
		protected void ReadDestructableWalls(NoxBinaryReader rdr)
		{
			...
			while (rdr.BaseStream.Position < finish)
			{
				...
				catch (Exception) { Debug.WriteLine("DestructableWalls read fail @" + String.Format("X:{0}, Y:{1}",x,y)); }
				num--;
			}
		...
		protected void ReadScriptObject(NoxBinaryReader rdr)
		{
			...
			Headers.Add(hed.name,hed);
			short version = BitConverter.ToInt16(hed.header, 0);
			Debug.WriteLineIf(version != 0x0001, "header for ScriptObject was not 0x0001, it was 0x" + version.ToString("x") + ".");
