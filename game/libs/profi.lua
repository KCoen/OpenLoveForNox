--[[
	...
	*Arguments are specified as: type/name/default.
	...
	Note: Poorly modified by http://github.com/CFusion 
]]
...
function ProFi:writeProfilingReportsToFile( reports, file )
	...
	local header = string.format( FORMAT_HEADER_LINE, "FILE", "FUNCTION", "LINE", "TIME", "RELATIVE", "CALLED", "MEMORY" )
	file:write( header )
 	for i, funcReport in ipairs( reports ) do
		...
		local mem = funcReport.gccountDelta
		local outputLine    = string.format(FORMAT_OUTPUT_LINE, funcReport.title, timer, relTime, count, mem )
		--    = gccountDelta --string.format(FORMAT_OUTPUT_LINE, funcReport.title, timer, relTime, count )
...
function ProFi:onFunctionCall( funcInfo )
	...
	funcReport.gccount = collectgarbage('count')
...
function ProFi:onFunctionReturn( funcInfo )
	...
	if funcReport.callTime then
		funcReport.gccountDelta = funcReport.gccountDelta or 0
		funcReport.gccountDelta = funcReport.gccountDelta + (collectgarbage('count') - funcReport.gccount)
